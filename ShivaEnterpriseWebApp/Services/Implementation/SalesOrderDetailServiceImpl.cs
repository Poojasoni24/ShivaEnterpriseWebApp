...
namespace ShivaEnterpriseWebApp.Services.Implementation
{
    public class SalesOrderDetailServiceImpl : ISalesOrderDetailServiceImpl
    {
        ...
        public async Task<(bool success, string message)> EditSalesOrderDetailDetailsAsync(List<SalesOrderDetail> salesorderdetail, string authToken)
        {
            try
            {
                //Create json string to prepare input for api                
                ...
                StringContent data = new StringContent(json, Encoding.UTF8, "application/json");
                var url = urlCollections["baseUrl"].ToString() + urlCollections["editsalesorderdetailUrl"] + "?id=" + salesorderdetail.FirstOrDefault().SalesOrderDetailId;
                var client = new HttpClient();
        ...
        public async Task<SalesOrderDetail> getsalesorderdetailbySalesOrderid(Guid salesorderId, string authToken)
        {
            var url = urlCollections["baseUrl"].ToString() + urlCollections["getsalesorderdetailbySalesOrderidUrl"] + "?salesorderId=" + salesorderId;
            var client = new HttpClient();
            ...
            //close out the client
            client.Dispose();
            SalesOrderDetail salesorderdetailDetail = null;
            try
            {
                salesorderdetailDetail = JsonConvert.DeserializeObject<SalesOrderDetail>(result);
                return salesorderdetailDetail;
            }
            catch (Exception ex)
            {
