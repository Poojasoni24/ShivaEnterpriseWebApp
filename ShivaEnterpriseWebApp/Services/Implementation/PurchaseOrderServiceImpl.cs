...
namespace ShivaEnterpriseWebApp.Services.Implementation
{
    public class PurchaseOrderServiceImpl : IPurchaseOrderServiceImpl
    {
        ...
        public async Task<(bool success, string value)> AddPurchaseOrderDetailsAsync(PurchaseOrder purchaseorder, string authToken)
        {
            try
            {
        ...
        public async Task<PurchaseOrder> GetPurchaseOrderById(Guid purchaseorderId, string authToken)
        {
            var url = urlCollections["baseUrl"].ToString() + urlCollections["getpurchaseorderlbyidUrl"] + "?purchaseorderId=" + purchaseorderId;
            var client = new HttpClient();
